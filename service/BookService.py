...
def get_books():
    ...
    try:
        curs = conn.cursor()             # 2. cursor() 객체를 사용하여 작업(노동자 생성)
        sql = "SELECT * FROM tbl_book;"  # 3. MariaDB 실행 SOL문(실행 X)
        curs.execute(sql)                # 4. cursor() 객체를 통해서 SOL문 실행(실행 O)
